...
int __init gpiochip_reserve(int start, int ngpio)
{
	...
	PRINTK("%s: reserved gpios from %d to %d\n",
...
int gpio_request(unsigned gpio, const char *label)
{
  ...
  if (chip == NULL) printk("chip == NULL\n");
	...
	if (status)
		PRINTK("gpio_request: gpio-%d (%s) status %d\n",
			gpio, label ? : "?", status);
	spin_unlock_irqrestore(&gpio_lock, flags);
	return status;
}
...
int gpio_direction_input(unsigned gpio)
{
	...
	if (status) {
		...
		if (status < 0) {
			PRINTK("GPIO-%d: chip request fail, %d\n",
