...
#define IRQ_POLL_PERIOD		      (msecs_to_jiffies(5))
#define IRQ_POLL_PERIOD_LONG		(msecs_to_jiffies(500))
...
#define ENC28J60_GPIO_IRQ  STM32_GPIO_PORTPIN2NUM(2,4) // PC4
static struct timer_list irq_poll_timer;
static volatile int counthi=0;
...
static void enc28j60_poll_handler(unsigned long arg)
{
  struct enc28j60_net *priv = (struct enc28j60_net *) arg;
  
    if (!gpio_get_value(ENC28J60_GPIO_IRQ))
      schedule_work(&priv->irq_work);

    if (counthi >= 200)
    {
      irq_poll_timer.expires += IRQ_POLL_PERIOD;
      add_timer(&irq_poll_timer);  
    }
    else
    {
      irq_poll_timer.expires += IRQ_POLL_PERIOD_LONG;
      add_timer(&irq_poll_timer);
    }
    
    if (gpio_get_value(ENC28J60_GPIO_IRQ)) 
      if (counthi < 300) counthi++;
    else counthi=0; 
...
static irqreturn_t enc28j60_irq(int irq, void *dev_id)
{
	 ...
	 */
	schedule_work(&priv->irq_work);
#ifndef CONFIG_STM32_GPIO_INT
       /* Clear the EXTI line 3 pending bit */
	stm32_exti_clear_pending(4);
#endif
	return IRQ_HANDLED;
}
...
{
  struct spi_stm32_slv *pdata = (struct spi_stm32_slv *)spi->dev.platform_data;
	struct net_device *dev;
	struct enc28j60_net *priv;
	int ret = 0;
	 ...
	 */
  printk("enc28j60: init irq_poll_timer\n");
	init_timer(&irq_poll_timer);
	irq_poll_timer.function = enc28j60_poll_handler;
	irq_poll_timer.expires = jiffies + IRQ_POLL_PERIOD_LONG;
  irq_poll_timer.data = (unsigned long)priv;
	//add_timer(&irq_poll_timer);
...
//                                ENC28J60_LINE_IRQ)
  ...
  //spi->irq = gpio_to_irq(ENC28J60_GPIO_IRQ);
  printk("enc28j60: IRQ = %d\n", spi->irq);
...
static int __init enc28j60_init(void)
{
	...
	printk("enc28j60: spi_register_driver\n");
