...
#define enc_enable() spi_xfer(spi1, 0, NULL, NULL, SPI_XFER_BEGIN)
#define enc_disable() spi_xfer(spi1, 0, NULL, NULL, SPI_XFER_END)
...
{
	spi_xfer(spi1, 8, &data, NULL, 0);
}

static unsigned char spi_read (void)
{
	unsigned char data;
	spi_xfer(spi1, 8, NULL, &data, 0);
	return data;
}
